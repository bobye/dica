% Examples of use of the moment matching topic modeling algorithms.
...
momtype = 'dica';
tic, A = spectral(SX, K, momtype); time = toc;
Dest = estimate_D(A);
...
momtype = 'lda';
tic, A = spectral(SX, K, momtype, c0); time = toc;
Dest = estimate_D(A);
err1 = l1_error(Dest, D);
...
tic, A = jd(SX, K, momtype, opts); time = toc;
Dest = estimate_D(A);
err1 = l1_error(Dest, D);
...
momtype = 'lda';
opts = 0; % default
tic, A = jd(SX, K, momtype, opts, c0); time = toc;
Dest = estimate_D(A);
err1 = l1_error(Dest, D);
...
momtype = 'dica';
tic, A = tpm(SX, K, momtype); time = toc;
Dest = estimate_D(A);
err1 = l1_error(Dest, D);
...
momtype = 'lda';
tic, A = tpm(SX, K, momtype, c0); time = toc;
Dest = estimate_D(A);
err1 = l1_error(Dest, D);
disp(['tpm (lda)       l1-error = ',num2str(round(err1*1000)/1000),',   time = ',num2str(time),' sec'])

% Warning: this script is only for the demonstrantion of the syntax of the
%          algorithms; do not draw too many conclusions from these toy
%          data; do not draw too many conclusions from one run of the
%          script - the errorbars always matter
